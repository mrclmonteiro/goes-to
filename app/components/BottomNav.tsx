...
const glassStyle = {
  ...
  boxShadow: '0 4px 24px rgba(0,0,0,0.15), inset 0 1px 0 rgba(255,255,255,0.35)',
}
...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-center gap-3 px-4"
      style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 20px)' }}>

      {/* Pill principal — mesma altura que o círculo (h-14 = 56px) */}
      <div className="flex items-center px-3 rounded-[28px] gap-1"
        style={{ ...glassStyle, height: '56px' }}>
        {mainTabs.map(tab => {
          const active = pathname === tab.href || pathname.startsWith(tab.href + '/')
          return (
            <Link key={tab.href} href={tab.href}
              className="flex flex-col items-center gap-1 px-4 rounded-[20px] transition-all"
              style={{
      ...
      <Link href="/busca"
        ...
        style={{
          ...
          border: `1px solid ${searchActive ? 'rgba(251,191,36,0.5)' : 'rgba(255,255,255,0.28)'}`,
        }}>
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
